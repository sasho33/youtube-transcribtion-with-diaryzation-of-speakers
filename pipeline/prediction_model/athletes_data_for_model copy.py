...
# After loading the JSON as a list
with open(STYLES_COMBO_RATES_FILE, encoding="utf-8") as f:
    raw = json.load(f)

# Find where the "style_vs_style" and "style_combos" lists start
style_vs_style_list = []
style_combo_list = []

for i, entry in enumerate(raw):
    if entry == "style_vs_style":
        style_vs_style_list = raw[i+1]
    if entry == "style_combos":
        style_combo_list = raw[i+1]

style_vs_style_dict = {}
style_combo_success_dict = {}

for entry in style_vs_style_list:
...
def get_style_combo_key(style1, style2):
    # Sorted to match the JSON convention for combos
    return " + ".join(sorted([style1 or "Unknown", style2 or "Unknown"]))


def get_style_vs_style_key(style1, style2):
    # Keys are in format: "style1 vs style2" (case sensitive!)
    return f"{style1 or 'Unknown'} vs {style2 or 'Unknown'}"

def get_combo_success_pct(style1, style2):
    key = " + ".join(sorted([style1 or "Unknown", style2 or "Unknown"]))
    return combo_success_pct_dict.get(key, None)

def get_style_vs_style_pct(style1, style2):
    ...
    for event in events:
        ...
        for m in event.get("matches", []):
            ...
            row = {
                ...
                # === NEW FIELDS BELOW ===
                "f1_style_combo_success_percent": style_combo_success_dict.get(
                   get_style_combo_key(
                       a1["pulling_style"][0] if a1.get("pulling_style") else "Unknown",
                       a2["pulling_style"][0] if a2.get("pulling_style") else "Unknown",
                    ), None
                ),
                "f2_style_combo_success_percent": style_combo_success_dict.get(
                    get_style_combo_key(
                        a2["pulling_style"][0] if a2.get("pulling_style") else "Unknown",
                        a1["pulling_style"][0] if a1.get("pulling_style") else "Unknown",
                    ), None
                ),
                "style_vs_style_success_percent": get_style_vs_style_pct(
                    a1["pulling_style"][0] if a1.get("pulling_style") else "Unknown",
                    a2["pulling_style"][0] if a2.get("pulling_style") else "Unknown",
                ),
